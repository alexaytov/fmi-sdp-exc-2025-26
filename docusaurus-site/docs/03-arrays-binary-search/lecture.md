...
### Свойства на Масивите (ЗАДЪЛЖИТЕЛНО!)

| Свойство | Описание | Последици |
|----------|----------|-----------|
| **Непрекъснато разпределение** | Елементите са един до друг в паметта | Бърз достъп, добра кеш локалност |
| **Фиксиран размер** | Размерът не може да се променя след създаването | Трябва да знаем размера предварително |
| **Хомогенни елементи** | Всички елементи са от един тип | Еднакво заемане на памет |
| **O(1) достъп** | Константно време за достъп по индекс | `arr[i]` е моментално |
...
**Защо е O(1)?** Адресът се изчислява с **една аритметична операция**, независимо от размера на масива!

### Двоично Търсене - Ключови Формули

**Условие:** Масивът **ТРЯБВА да е сортиран!**
...
| Брой елементи | Линейно O(n) | Двоично O(log n) | Подобрение |
|---------------|--------------|------------------|------------|
| 100 | 100 | 7 | **14x** |
| 1,000 | 1,000 | 10 | **100x** |
| 1,000,000 | 1,000,000 | 20 | **50,000x** |
| 1,000,000,000 | 1,000,000,000 | 30 | **33 милиона x** |

### Основни Операции - Сложност

| Операция | Масив | Сложност |
|----------|-------|----------|
| Достъп по индекс | `arr[i]` | **O(1)** |
| Търсене (несортиран) | Линейно | **O(n)** |
| Търсене (сортиран) | Двоично | **O(log n)** |
| Вмъкване в края | `arr[size++] = x` | **O(1)** (ако има място) |
| Вмъкване в средата | Shift всички след | **O(n)** |
| Изтриване | Shift всички след | **O(n)** |

### Custom Array Class - Ключови Елементи

```cpp
template <typename T>
class CustomArray \{
private:
    T* data;         // Динамичен масив
    size_t capacity; // Заделена памет

public:
    // ЗАДЪЛЖИТЕЛНИ методи за exam:
    CustomArray(size_t n);           // Конструктор
    ~CustomArray();                  // Деструктор - free памет!
    CustomArray(const CustomArray&); // Copy constructor
    CustomArray& operator=(const CustomArray&); // Assignment

    T& operator[](size_t i);         // Достъп без проверка
    T get(size_t i) const;           // Достъп с проверка
    void set(size_t i, const T& val); // Задаване
    size_t getSize() const;
\};
```

### Често Срещани Грешки при Binary Search

```cpp
// ❌ ГРЕШНО - Безкраен цикъл
while (low < high) \{ ... \}  // Пропуска последен елемент

// ✅ ПРАВИЛНО
while (low <= high) \{ ... \}

// ❌ ГРЕШНО - Overflow при големи числа
int mid = (low + high) / 2;
...
// ❌ ГРЕШНО - Безкраен цикъл
low = mid;  // или high = mid;

// ✅ ПРАВИЛНО
low = mid + 1;  // или high = mid - 1;
```
...
**❌ Неподходящ:**
- Несортиран масив (сортирането струва O(n log n))
- Данните се променят често
- Малък брой елементи (n < 10)
